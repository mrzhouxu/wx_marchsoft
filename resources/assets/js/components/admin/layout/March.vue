		...
		methods:{
			deleteRow(index, rows) {    //删除信息
				this.$confirm('确定删除吗？', '提示', {
                      ...
                      rows.splice(index, 1);
                     axios.post('/admin/delete/'+no).then(res=>{
                        if(res.data.code == 0){
                            this.$message({
                            type: 'success',
                            message: '删除成功！'
		      ...
		      handleCurrentChange(val) {   //分页
		      	var self = this;
		          axios('/admin/change_page?page='+val).then( function(res) {
		          		if (res.data.code==1){
		          			this.$message({
		                            type: 'error',
		                            message: '查询失败!'
		       ...
		       handleIconClick(ev) {   //查询信息
		     		 ...
		     		 if (this.input&&this.value) {
		     		 	...
		     		 	axios.post('/admin/search',postdata).then( function(res){
		     		 			if (res.data.code==1) {
		     		 				this.$message({
		                            type: 'error',
		                            message: '查询失败!'
		   		...
		   		back(){
		   			var self = this;
					axios.get('/admin/march/people').then(function(res){
		              self.tableData=res.data.data;
		              self.total=res.data.total;
		              self.value='';
					}, function(){
