...
<script>
    ...
    export default {
        ...
        methods: {
            submitContent() {
                ...
                if (this.content != this.contentJudge) {
                    axios.post('/Road/updateRoad', { content: this.content }).then(
                        res => {
                            if (res.data.code == 0) {
                                self.$message({
                                    message: '修改成功',
        ...
        mounted: function () {
            let update = this;
            axios.get('/Road/getRoad').then(
                res => {
                    if (res.data.code == 0) {
                        update.content = res.data.result.content;
                        update.contentJudge = this.content;
                        update.$refs.ue.setUEContent(this.content);
                    }else{
                        self.$message.error('哎呀妈呀！出错了，请刷新一下。');
                    }
                }
