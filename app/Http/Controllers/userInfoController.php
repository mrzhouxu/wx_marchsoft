...
class userInfoController extends Controller
{
    ...
    /**
     * @param Request $request 获取请求数据
     * @return \Illuminate\Http\JsonResponse 返回json格式数据用于判断删除是否成功
     */
    ...
    public function new_user_msg(Request $request)
    {
        ...
        if ($file != '') {
            if ($file->isValid()) {
                ...
                $fileName = getFilename('jpg');
                $user_info['avator'] = $fileName;
                $path = $request->file('avator')->storeAs('/', 'm_' . $fileName, 'test');
                $update_boo = march_user::add_user_mesg($user_info);
                return $update_boo ? responseToJson(0, '添加成功') : responseToJson(1, '添加失败');
